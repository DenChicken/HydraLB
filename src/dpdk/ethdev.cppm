...
constexpr std::uint16_t RX_QUEUES_DEFAULT = 1;
constexpr std::uint16_t TX_QUEUES_DEFAULT = 1;
...
struct DeviceConfig {
    std::uint16_t rx_queues = RX_QUEUES_DEFAULT;
    std::uint16_t tx_queues = TX_QUEUES_DEFAULT;
...
public:
    ...
    std::expected<void, std::string> configure(const DeviceConfig& config) {
        ...
        int info_ret = ::rte_eth_dev_info_get(static_cast<std::uint16_t>(id_), &dev_info);
        if (info_ret < 0) {
    ...
    std::expected<void, std::string> setup_rx_queue(QueueId queue_id, Mempool& pool) {
        int ret = ::rte_eth_rx_queue_setup(
            static_cast<std::uint16_t>(id_),
            static_cast<std::uint16_t>(queue_id),
            rx_descriptors_,
            ::rte_eth_dev_socket_id(static_cast<std::uint16_t>(id_)),
            nullptr,
            pool.raw());
    ...
    std::expected<void, std::string> start() {
        int ret = ::rte_eth_dev_start(static_cast<std::uint16_t>(id_));
        if (ret < 0) {
            return std::unexpected(
                std::format(
                    "Failed to start eth device {}: {}",
                    static_cast<std::uint16_t>(id_),
                    ret));
        }

        ::rte_eth_promiscuous_enable(static_cast<std::uint16_t>(id_));
